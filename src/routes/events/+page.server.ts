...
export const load = async ({ locals }) => {
	...
	const live_events = event_team_count
		.map((event) => {
			const player_count =
				event_player_count
					.find((e) => e.id === event.id)
					?.teams.reduce((acc, team) => acc + team._count.channels, 0) || 0;
			if (
				moment().isBetween(event.start_date, event.end_date) &&
	...
	const upcoming_events = event_team_count
		.map((event) => {
			const player_count =
				event_player_count
					.find((e) => e.id === event.id)
	...
	const completed_events = event_team_count
		.map((event) => {
			const player_count =
				event_player_count
					.find((e) => e.id === event.id)
